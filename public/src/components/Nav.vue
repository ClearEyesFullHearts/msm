...
<template>
    <nav v-if="authStore.user" class="navbar navbar-expand navbar-dark bg-dark">
        <div class="navbar-nav mr-auto">
            ...
            <router-link to="/messages" class="nav-item nav-link">Inbox</router-link>
            <router-link to="/memory" class="nav-item nav-link">MemoryBox</router-link>
        </div>
        <div class="navbar-nav mr-auto">
            <button @click="incinerate()" class="btn btn-outline-danger">Incinerate @{{ authStore?.user?.user?.username }}</button>
        </div>
        <div>
            <span class="navbar-text">Your session will expire in {{ countDownMsg }}</span>
            &nbsp;
            <button @click="authStore.logout()" class="btn btn-outline-light">Logout</button>
        </div>
    </nav>
</template>