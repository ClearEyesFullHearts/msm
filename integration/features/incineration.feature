...
Scenario: A member is removed from its groups when it is incinerated
  Given `RANDOM_USER.1` creates a group firstNameGroup for ["`RANDOM_USER.15`"] with index 0
  Given `RANDOM_USER.3` creates a group firstNameGroup for ["`RANDOM_USER.15`", "`RANDOM_USER.4`", "`RANDOM_USER.1`"] with index 1
  Given `RANDOM_USER.4` creates a group firstNameGroup for ["`RANDOM_USER.15`", "`RANDOM_USER.1`"] with index 2
  And I save `RANDOM_USER.15`
  And I am existing `RANDOM_USER.15`
  And I set signature header
  When I DELETE /user/`RANDOM_USER.15`
  Then response code should be 200
...
Scenario: An admin incinerate its groups if they are the only admin
  Given `RANDOM_USER.1` creates a group firstNameGroup for ["`RANDOM_USER.15`", "`RANDOM_USER.3`"] with index 0
  Given `RANDOM_USER.3` creates a group firstNameGroup for ["`RANDOM_USER.15`", "`RANDOM_USER.4`", "`RANDOM_USER.1`"] with index 1
  Given `RANDOM_USER.4` creates a group firstNameGroup for ["`RANDOM_USER.15`", "`RANDOM_USER.3`"] with index 2
